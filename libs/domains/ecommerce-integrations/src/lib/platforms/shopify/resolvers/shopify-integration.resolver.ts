...
export class ShopifyIntegrationResolver {
  ...
  @Mutation(() => ShopifyStore, {
    ...
    description:
      'Connect a Shopify store to the platform by providing shop domain and access token',
  })
  ...
  disconnectShopifyStore(
    @Args('id', { description: 'ID of the Shopify store to disconnect' })
    id: string,
  ): Promise<ShopifyStore> {
  ...
  async syncShopifyOrders(
    @Args('storeId', { description: 'ID of the Shopify store' })
    storeId: string,
  ): Promise<string[]> {
    const orders = (await this.shopifyIntegrationService.syncOrders(
      storeId,
    )) as Array<{ id: string }>;
